...
def compute_f1(preds, y):
    ...
    y_ind = [i for i,e in enumerate(rounded_preds.cpu().numpy()) if max(e)<0.5]
    print("length of removed preds: ", len(y_ind))
#     y_pred = np.delete(y_pred, y_ind)
#     y_true = np.delete(y_true, y_ind)
...
#     result2 = precision_recall_fscore_support(y_true, y_pred, average='macro')
    ...
    return acc, f1_average, result[2][0], result[2][1], y_ind